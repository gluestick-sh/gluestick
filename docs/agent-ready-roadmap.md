...
> 闭环（2026-09-25）：上表为 Phase 1 开始时的快照。P0/P1/P2 三批补完后，除 `completion`（P3，明确不做）外**全部命令已有 `--json`**；最后一条遗漏是 `bucket remove`（Phase 1 之后的复测才暴露：它把人类文案直接打到 stdout），本轮已补 JSON + 类型化错误码 `bucket_not_found`，`docs/json-schema.md` 同步覆盖 bucket/cache/path/hold/reset/home/update-check。`engine` 为内部调试入口，不属对外契约。
...
| `glue_bucket_remove` | engine.Bucket | 删桶（destructive：走同一 confirm + deny/protected 闸门） |
...
| `destructive` | uninstall/reset/cache clear/bucket remove | 默认需确认 + **可被 denylist 硬拦截** |
...
- `protected`：**目标级保护**——受保护的包在 MCP 路径不可 uninstall/reset，受保护的 bucket 名不可 `bucket_remove`（如 `protected: ["main"]` 保住主桶）；人类经 CLI 可越过（人机权限不对称：agent 低权、人高权）。
...
| 2026-09-25 | **调度式链校验 + 外部锚点 + doctor/env 审计行 + 仓库行尾统一为 LF**：新增 `audit.verify_interval_hours`（默认 24h，`off`/`never`/`disabled` 关闭），每次审计追加后按间隔重算链——健康时只刷新 `<root>/logs/.audit-verify` 时间戳（静默 tripwire，不产生噪音行，保持既有行数断言有效），检出篡改则写一条 `audit_verify`（`status=broken`、`details.brokenAt`/`scheduled`）并打 stderr 警告（`recordAuditRow` 拆分以避免自触发）；`AuditVerifyResult` 增 additive 字段 `head`（最新条目 hash，供外部固定）与 `expected`；`glue audit verify --expect <hash>` 做外锚比对（不匹配 → `ok=false`/`reason="head mismatch"`/exit 1，`auditHashShort` 用于人视图与提示）；`glue doctor`/`glue env` 分别落 `doctor`/`env` 审计行（`RecordAgentDoctorActivity` 带 agentReady/score/failing checks、`RecordEnvironmentActivity`）；`glue config get/set/unset/list` 支持 `audit.verify_interval_hours`（非整数 → `invalid_argument`）。另修仓库卫生：新增 `.gitattributes`（`* text=auto eol=lf` + 二进制标记）并设 `core.autocrlf=false`，195 个 CRLF 文件归一为 LF（零内容变更、BOM 保留），根治 `gofmt -w` 在 CRLF 检出的工作区造成的 249 个 status 幽灵条目（实际内容差异仅 70 个） | 新增 core `TestAuditVerifyHead_tracksNewestEntry`、`TestScheduledAuditVerify_quietWhenHealthy`/`_runsWhenDue`/`_reportsBrokenChain`/`_disabledByConfig`，CLI `TestAuditVerify_expectPinnedHead`、`TestDoctorAndEnv_recordAuditRows`；build/vet/test 绿 |
| 2026-09-25 | **`bucket remove --json` + 全命令 JSON schema 补全（§2.3 与 §3.3 收尾）**：逐命令实测发现 `glue bucket remove` 是唯一仍把人类文案直接写到 stdout 的写命令（`✓/✗` 行混进管道，违反“`--json` 只有一份 JSON 文档”的契约）——补 `jsonOperationResult("bucket_remove", items)`（`{command, ok, results[{ref,error,code,hint}]}`），成功项仍写入搜索索引并落审计，部分失败时成功项照常生效、`ok=false` 退出 1；core 新增类型化 `apperr.ErrBucketNotFound` / `BucketNotFound`（文案保持 `bucket not found: <name>` 不变），`bucket.Registry` 的 `Remove`/`Get`/`GetManifestPath` 改用它，CLI `jsonErrorInfo` 映射为稳定错误码 `bucket_not_found`（hint: `glue bucket list shows installed buckets`）；JSON 模式下 git 缺失警告改走 stderr（stdout 保持纯净，与 `bucket list` 在 JSON 模式不污染 stdout 的既有做法一致）；`docs/json-schema.md` 从 10 个命令扩到全量：bucket add/update/check/known/remove、cache list/clear(±--all)/gc/rebuild、path show/check/setup、hold/unhold/reset、home、update 检查态（`upToDate`/`matched`）、错误码清单与 `list` 空值 `null` 的已知不一致说明 | 新增 core `TestBucketNotFoundIs`、`TestRegistryRemove`（类型化错误 + 目录清理），CLI `TestRunBucketRemoveJSON_missingBucket`（code=`bucket_not_found`、exit 1、stdout 纯 JSON）/`_success`（目录删除、exit 0）/`_partialFailure`（一成功一失败 → `ok=false`、成功项已生效）；`go build`/`go vet`/`go test ./core/apperr ./core/bucket ./cli/glue` 绿；实机冒烟（temp 根）：`--json bucket remove no-such-bucket` → `bucket_not_found`+hint+exit 1、`--json bucket remove main` → `ok=true`+exit 0 且 `bucket list` 随之清空、文本模式文案逐字不变 |
| 2026-09-25 | **MCP `glue_bucket_remove`（destructive 闸门补全）+ 桶操作审计补记 + `list` 空值统一**：新增 MCP tool `glue_bucket_remove`（§4.6.2 destructive 级）——默认只发 confirm token，`glue_confirm` 才真正删除；`agent.policy.deny` 命中 `bucket_remove` 直接拒绝（即使 `auto_yes`），`protected` 由「包级」扩为「目标级」：`protected: ["main"]` 可保住主桶（`mcpProtectedOps`/`mcpProtectedNoun`，包侧文案零变化）；执行层 `executeMCPBucketRemove` 删桶 + 清搜索索引，错误按 CLI 同源映射为 `{code:...}`（`bucket_not_found`）；**审计保真**：新增 `recordMCPBucketActivity`，MCP 的 bucket add/update/remove 三条路径改用请求 context 落完成行，`glue audit list --source mcp` 现在能看到 `bucket_add`/`bucket_update`/`bucket_remove` 的 success 行（此前 add/update 在 MCP 路径完全没有完成行，只有 confirm 决策链）；`glue list`/`list --all` 空结果由 `null` 改为 `[]`（nil slice 归一，兑现上一轮文档里标注的“已知不一致”），`docs/json-schema.md` 去 caveat，`docs/mcp.md` 更新 tool 表/策略/审计说明，roadmap §4.2 与 §4.6.2 同步补 `bucket_remove` | 新增 CLI `TestMCPBucketRemove_confirmExecutes`（pending→confirm→目录消失→`source=mcp`+actor 的 success 审计行）、`TestMCPBucketRemove_deniedByPolicy`（deny+auto_yes 仍拒绝且目录保留）、`TestRunListJSON_emptyPackagesIsArray`/`TestRunListAllJSON_emptyPackagesIsArray`；扩展 `TestDecideMCPPolicy`（protected 桶 + deny 桶删除）、`TestMCPWriteTools_listed`、`TestMCPNewWriteToolsPending`；build/vet/test 绿。**附带修复**：`glue mcp` 在客户端关闭 stdin 时曾以 exit 1 + stderr `server is closing: EOF` 退出（SDK 的 internal jsonrpc2 错误既不是 `io.EOF` 也不是 `mcp.ErrConnectionClosed`，原有判断漏掉）——新增 `isMCPShutdown` 归类（导出 sentinel + 消息兜底），关闭管道现为 exit 0 且 stderr 干净 | 另新增 `TestIsMCPShutdown`（正常关闭 9 例 / 真错误 3 例）；**真机 stdio 联调**（`glue-alpha.exe --root <temp> mcp`，管道喂 JSON-RPC）：`tools/list` 回 14 个 tool（含 `glue_bucket_remove`）、exit=0、stderr 干净；两进程两阶段跑通 destructive 流程——进程 A `glue_bucket_remove demo` 返回 `pending`+32 位 token，进程 B 用该 token `glue_confirm` 成功删桶（`{"command":"bucket_remove","name":"demo","ok":true}`，目录消失），`glue audit list --source mcp` 呈完整链路 `pending → granted → success` 且 actor=`cline/3.7` |
| 2026-09-25 | **IDE 客户端接入测试就绪（Phase 2 验收路径）**：`docs/mcp.md` 新增完整接入章节——「指向哪个命令」对照表（**默认 = 本工程 `C:\gluestick-sh\glue-alpha.exe` + `["mcp"]` → 数据根 `~/.glue-alpha`**，另列显式 `--root`、一次性沙箱、已装 `glue`；含数据根差异与**旧安装无 `mcp` 子命令**的踩坑提示）、五家客户端配置位置与片段（Cline 面板 + `cline_mcp_settings.json`、Claude Code `claude mcp add … --scope user -- …`、Claude Desktop `%APPDATA%\Claude\claude_desktop_config.json`、Cursor `~/.cursor/mcp.json`、VS Code `.vscode/mcp.json` 顶层键为 `servers`（非 `mcpServers`），可用 `code --add-mcp`（PS 5.1 需 `--%` 防止引号被吞）或命令面板 `MCP: Add Server → Command (stdio)`；**无 URL 形态**（glue 只提供 stdio；VS Code 的 `url` 字段属远程 HTTP/SSE，本路线图 §4.1 明确不做）、**无需 IDE 的 JSON-RPC 预检脚本**（UTF-8 无 BOM + 保活管道，**不带 `--root`** 与客户端配置同根）、Phase 2 验收场景（doctor→install→confirm→path_check + 审计链路）与排障表；新增**真机 stdio E2E 测试** `cli/glue/mcp_stdio_e2e_test.go`（`mcp.CommandTransport` 拉起真实二进制，**默认跑 dev 根 `%USERPROFILE%\.glue-alpha`**（`GLUE_MCP_ROOT` 可覆盖）、**拒绝跑真装根 `~/.glue`**，断言 tool 面、`[]` 载荷契约、dev 根 shim 路径、pending→confirm 删桶、关闭 stdin 后 **exit 0**；无 `GLUE_MCP_BIN` 自动 skip，默认测试集保持无外部依赖），`AGENTS.md` 增该命令 | 顺带修两处载荷不一致（真机探针发现）：MCP `glue_list` 空结果 `packages:null` → `[]`、CLI `glue info` 全部失败时 `packages:null` → `[]`，新增 `TestMCPCall_listEmpty` 断言与 `TestInfoJSON_allFailedEmptyPackages`；**本机实机接线与验证**：Claude Desktop（合并进既有 `claude_desktop_config.json`，已留 `.bak-glue` 备份）、Cursor `~/.cursor/mcp.json`、VS Code `%APPDATA%\Code\User\mcp.json` 三处写入 `C:\gluestick-sh\glue-alpha.exe` + `["mcp"]`；**以配置文件为唯一真源**逐个回读 `command`/`args` 再做 JSON-RPC 握手，三家均 `exit=0`、14 个 tool、`serverInfo.name=glue`、stderr 空；`glue-alpha.exe --json audit list --source mcp` 显示 `tool_call`/`bucket_remove` 的 `pending → granted → success` 链路与 actor；E2E 测试对 dev 根 PASS 且 `demo` 桶用完即删，真装 `~/.glue` 无审计文件、全程零改动（Cline 未安装故仅文档说明） |
| 2026-09-25 | **`glue doctor` 的 `agent_policy` 检查由硬编码改为读配置**（由一次真实 `glue_doctor` 调用暴露）：该 advisory 检查此前固定返回过期结论 `gates (deny/protected/confirm) arrive with the MCP phase` + `data.configured=false`（连 data root 都不读），而 Phase 2 早已上线策略闸门并会以 `denied_by_policy` 生效——agent 读到的是"闸门还不存在"的错误事实。改为 `agentCheckPolicy(root)` 读 `config.json` 的 `agent.*`：显式配置（deny / protected / mode≠confirm / auto_yes）→ **pass**，detail 显示 `mode=… auto_yes=… deny=N protected=N`；纯默认 → 仍为 advisory，但文案为 `defaults in effect: mode=confirm, no deny/protected entries`，hint 改成可执行的 `glue config set agent.policy.deny\|protected\|mode`（新增消息键 `agent.policy.defaults`） | 新增 core `TestAgentCheckPolicy_reflectsConfig`（默认态 + deny / protected / mode=auto / auto_yes，5 例全绿）；`gofmt`/`go vet` 干净；`go test ./cli/glue ./core/engine ./core/message -count=1` 全绿；实机 MCP `glue_doctor` 复核 detail/hint/data 均已修正 |
| 2026-09-25 | **`agents` 检查新增客户端矩阵（方案 B）+ `agent_policy` 改为读配置**：`agentCheckAgents()` 的判定保持"PATH 上的 agent CLI 共存"（不改变 `agentReady`/blocking/score），新增 additive `data.clients` —— 按 `docs/mcp.md` 文档的同一张矩阵探测 5 类 MCP 客户端（Cline / GitHub Copilot Chat / Cursor agent / Claude Desktop / Claude Code profile），每行给 `kind`(`cli`\|`ide-extension`\|`desktop`)、`detected`、`evidence`(命中路径)、`wired`(其 MCP 配置里是否存在 `glue` 条目；VS Code 用 `servers` 键，其余 `mcpServers`)、`config`；探测根可注入（`agentClientEnv`），非 Windows/空环境安全返回全 `detected:false`；人视图在 `agents` 下新增每个客户端一行（`installed, glue MCP registered` / `installed, glue not registered (<config>)` / `not installed`），用于解释"机器上明明有 agent 却报 CLI not found"；`docs/json-schema.md` 记录 `data.clients` schema 与"不影响 verdict"承诺，`docs/mcp.md` 说明 doctor 会报告客户端接线状态；另把 `agent_policy` 检查由硬编码改为读 `config.json`（显式配置→pass，默认→advisory + 真实文案与可执行 hint，新增消息键 `agent.policy.defaults`） | 新增 core `TestProbeAgentClients`（Cline 已装已接入 / Cursor 已装未接入 / Claude Desktop 有配置无 servers / Copilot 与 Claude Code 未装，5 行矩阵断言）、`TestProbeAgentClients_emptyEnv`、`TestAgentCheckAgents_carriesClients`、`TestAgentCheckPolicy_reflectsConfig`；CLI `TestWriteAgentCheckRow_rendersClients`（三种客户端状态行 + hint 并存）；`gofmt`/`go vet` 干净；本机实机输出：`— Cline not installed` / `✓ GitHub Copilot Chat|Cursor agent|Claude Desktop installed, glue MCP registered` / `⚠ Claude Code (profile) installed, glue not registered (C:\Users\xuc\.claude.json)` / `agent policy: defaults in effect: mode=confirm…` |
| 2026-09-25 | **`buckets` 检查的原因分级与可读 detail（+ 更正一处误报）**：`agentCheckBuckets` 的判定抽成纯函数 `bucketsCheckVerdict(bucketCount, indexed, total, indexReady)` 便于单测，并补齐此前只有 detailKey 没有 detail 的空文案：空桶 → `no buckets installed` + `glue bucket add main`；**桶已装但索引为空/未就绪** → `no manifests indexed from N bucket(s)` / `N of M bucket(s) indexed, K package(s)` + 新 hint `glue cache rebuild …（如仍失败，可能是其他 glue 进程仍占用数据根）`（新增消息键 `agent.hint.buckets_rebuild`；此前这两种情况都误发"Run glue bucket add main"，而桶其实装好了）；`data` additive 增 `buckets_indexed` | 观察来源：我早前探针残留的两个 `glue mcp` 进程运行期间，doctor 的 `buckets`（blocking）报 `index_ready=false/manifest_count=0` 且 hint 误导，杀掉残留进程后恢复 `index_ready=true/manifest_count=4026`、`agentReady=True/score=82`；新增 core `TestBucketsCheckVerdict`（4 个分支 + "已装未索引不得给 add 提示"断言）；实机复核：空桶 temp 根 → `no_manifests` + 新 hint + `buckets_indexed:0`；dev 根 → `pass`、`2 buckets, 4026 packages`、`buckets_indexed:2`；另更正：人视图此前并非空白（`formatDoctorDetail` 会回退渲染 detailKey 文案），空的是 JSON 的 `detail` 字段 |
| 2026-09-25 | **更正上一轮 `buckets` hint + `--fix` 按原因分派（诚实性）**：查证后发现上一轮我给 `index_not_ready`/`no_manifests` 写的 hint `glue cache rebuild` **是错的**——`cache rebuild` 重建的是"已装包内容缓存索引"（扫 apps 硬链接），与桶的**搜索索引**无关；搜索索引是 engine 启动时在内存里建的（`e.SearchIdx` + `SearchIdxReady`），所以"已装桶但索引未就绪"是**瞬时构建状态**而非磁盘故障。据此改正并补齐三处一致性：① hint 拆成 `agent.hint.buckets_index_busy`（"等一会儿再跑 glue doctor；若持续，可能是其他 glue 进程占用数据根"）与 `agent.hint.buckets_no_manifests`（"glue bucket update 重新拉取 manifests"）；② `nextActions` 由 `agentActionForCheck(check)` 改为按原因给命令（空桶 `glue bucket add main` / 索引忙 `glue doctor` / 无 manifests `glue bucket update`）；③ `doctor --fix` 新增 `doctorFixStepForCheck` 解析器（fix 计划、JSON `fixes[]` 与人视图标题共用一份真相）与新动作 `reindex_buckets`（`fixReindexBuckets`，逐桶 `LoadSearchIndexBucket`，**离线可执行**）与 `bucket_pull`（`fixPullBucketManifests`，`UpdateSilent`+`ReloadBuckets`，`--offline` 跳过并给原因）；此前"已装桶但索引空"会走 `bucket_add`，而 `fixAddMainBucket` 对已存在的 main 直接早返回 `applied=true`——即**谎报已修复** | 新增 core `TestAgentActionForCheck`（5 例）、改写 `TestBucketsCheckVerdict` 的 hint 期望（index busy / no manifests 分流）；CLI 新增 `TestDoctorFixStepForCheck_bucketReasons`（4 例）、`TestDoctorFixTitleForResult`（5 例）、`TestFixReindexBuckets`（本地 bucket → `PackageCountsByBucket[demo]==1`）、`TestFixPullBucketManifests_offline`（离线跳过并给出原因）；实机复核：空桶 temp 根 `--json doctor --fix --offline` → `fixes[] = {check:buckets, action:bucket_pull, applied:false, error:"offline: skipped (run glue bucket update without --offline)"}`（**不再谎报 applied**），人视图 `✗ Refresh bucket manifests — offline: skipped …`，`Next: glue path setup; glue bucket update`；dev 根不受影响（`pass`） |

**Phase 1 待办遗留**：
- ~~search/list/info 的 JSON Schema 文档固化（§3.3 P1，纯文档）~~ → 2026-09-25 完成：`docs/json-schema.md` 已覆盖全量命令（含 `glue doctor` 默认视图与 `glue env` 传统 schema），随本轮把 bucket/cache/path/hold/reset/home/update-check 一并补齐。
- ~~`glue list` 空结果输出 `null`~~ → 2026-09-25 已修：`list`/`list --all` 空结果统一为 `[]`（与 cache/bucket 一致）。
